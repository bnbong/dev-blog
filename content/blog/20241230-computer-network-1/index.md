...
현대에서는 정말 다양한 네트워크 장비가 존재합니다. 각 네트워크 장비는 제조사가 다르고 모델이 다르며, 심지어 계열이 같은 네트워크 장비라고 하더라도 버전이 다를 수 있습니다. 이러한 네트워크 장비들이 서로 통신하려면 각 장비가 상대방의 프로토콜을 알고 있어야 합니다.

그러나 서로 다른 환경에 놓인 네트워크 장비들이 상대방의 상황을 완벽하게 이해하기란 불가능에 가깝습니다. 그래서 프로토콜을 이용해 이를 추상화하여, 각 장비가 상대방의 상황을 이해할 수 있도록 만듭니다.
...
통신을 수행하는 두 네트워크 장비는 서로 대응하는 프로토콜 계층끼리 데이터를 주고받습니다.

![peercommunicate](peercommunicate.png)

A 네트워크 장비가 B 네트워크 장비와 통신하는 상황이라면, A 장비의 IP 계층에서 캡슐화된 데이터 패킷이 B 장비에 도달하고, B 장비의 IP 계층에서 디캡슐화된 뒤에 처리됩니다.
...
캡슐화는 상대방에게 보내려는 데이터를 전송하기 위해 각 계층에서 헤더를 추가하는 작업이고, 디캡슐화는 상대방에게서 받은 데이터를 원래 형태로 복원하는 작업입니다.

기본적으로 네트워크 장비의 각 계층은 다른 계층의 데이터 구조를 전혀 건드리지 않는다는 점이 원칙입니다. 따라서 원본 데이터 구조를 변경하지 않고, 단순히 헤더를 덧붙여서 필요한 정보만을 그 헤더에 기록합니다.

각 TCP 계층에서 캡슐화가 수행되면 다음 사진과 같이 계층별 헤더가 추가됩니다.

![encapsule](encapsule.png)

이렇게 캡슐화된 데이터는 상대방 네트워크 장비에서 각 계층의 헤더가 제거되면서 디캡슐화가 수행됩니다. 캡슐화와 마찬가지로, 디캡슐화를 수행하는 계층이 자신이 아닌 다른 계층의 헤더를 제거하는 동작은 불가능하도록 설계되어 있습니다.

</br>

TCP/IP 링크 계층까지 캡슐화가 완료되어 외부로 나가는 데이터 프레임은 전체적으로 다음과 같은 구조로 파싱됩니다.

![dataframe](dataframe.png)

위의 사진은 사람이 이해하기 쉽도록 표현한 것이고, 실제로는 헤더를 포함한 모든 데이터가 컴퓨터가 읽을 수 있는 형태로 변환됩니다.
