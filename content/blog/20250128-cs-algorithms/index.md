...
대학생 4학년으로 올라오면서 기업 코딩 테스트에 반드시 알아야 할 자료구조와 알고리즘을 블로그 포스팅으로 정리해보려 합니다.

지난 세월 동안 알고리즘과 담을 쌓고 살아왔기 때문에 이전에 배웠던 알고리즘과 자료구조를 한번 정리하는 시간이 필요했습니다.

여러 조사를 하고 백준 같은 문제 풀이 사이트에서 직접 문제를 풀어보면서, 자주 등장하고 중요하다고 생각되는 내용을 코드 중심으로 정리해보았습니다.

문제들은 Python으로 풀고 있기 때문에 예제 코드들은 모두 Python으로 작성하였습니다.
...
배열은 연속된 메모리 위치에 저장된 요소들의 모음입니다.

python의 경우, 다른 자료구조 보다 배열을 사용해서 탐색을 하게 되면 성능이 떨어지기 때문에 해시맵 등 다른 자료구조를 사용해서 구현하는 것이 좋습니다.
...
python에서 집합을 선언할 때는 `{}` 또는 `set()` 혹은 `dict()` 을 사용합니다.

선언 형태에 따라 사용할 수 있는 내장 메서드가 다릅니다. 상황에 알맞는 구조를 선택해서 구현하면 됩니다.
...
그래프 탐색의 경우, 베이스가 되는 BFS, DFS 구현체 자체를 어느정도 외우고 들어가야 문제를 어느정도 접근하기 용이합니다.

알고리즘 문제들은 풀고 있으면 고등학교 수학 문제 푸는 느낌이라 여전히 달갑지는 않습니다ㅠㅠ
