...
당시 백엔드 경험이 부족했던 터라 Django가 제가 고를 수 있는 가장 나은 스택이기도 했습니다. 같은 그룹웨어 서비스의 핵심인 커뮤니티 서비스 'Hub'가 Django로 구현되어 있어서, 기술 스택을 통일하려는 의도도 있었습니다. 새로운 기술을 시험하기보다 빠르게 신뢰할 만한 API를 제공하는 쪽이 더 중요했습니다.

## 구현 포인트

README의 구조를 기준으로 공통 모듈과 예외 처리, 권한 처리, 문서 정리를 각각 분리해 작업했습니다.
페널티 시스템이 작아 보여도 이후에 비슷한 내부 서비스와 규칙을 공유할 가능성이 높았기 때문입니다.
