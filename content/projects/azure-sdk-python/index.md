...
겉으로 보면 작은 수정 같지만, 이 경험에서 실제로 얻은 것은 코드 한 줄이 아니라
대형 오픈소스 저장소의 구조와 리뷰 프로세스를 직접 경험해 보았다는 점입니다.
...
- 코드 변경량보다 리뷰어가 이해하기 쉬운 변경 단위를 만드는 일이 더 중요하다는 점을 배웠습니다.

## 아쉬웠던 점

- Azure SDK Python 같은 초대형 프로젝트에서는 코드 품질과 개선 방향을 MS 내부 회의에서 논의한 뒤 Auto Gen이라는 코드 자동 생성 도구가 그 결정을 반영합니다. 외부 기여자는 이러한 맥락까지 파악하기 어려웠고, Auto Gen 태그가 붙은 코드는 오픈소스임에도 외부 기여자의 코드 기여를 막아 두었기 때문에 실질적으로 기여할 수 있는 코드가 많지 않았습니다.
