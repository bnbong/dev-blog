...
날씨 서비스는 이미 많지만, 저는 사용자가 매번 앱을 열어 확인하는 방식보다 메신저 안에서 바로 정보를 받는 경험을 더 중요하게 생각했습니다. 그래서 별도의 모바일 앱 대신 카카오톡 채널과 챗봇을 결합한 구조를 선택했습니다.

핵심 흐름은 다음과 같습니다.

- 외부 날씨 데이터를 수집합니다.
- 수집한 데이터를 지역과 시간을 기준으로 정리합니다.
- 정리한 정보를 카카오톡 채널 사용자에게 전달합니다.
...
메신저 봇은 "항상 켜져 있어야 한다"는 점이 중요했기 때문에, 개발 환경보다 운영 안정성을 먼저 고민하게 만드는 프로젝트였습니다.
...
- 메신저 기반 서비스는 API 서버 하나만 잘 만든다고 해서 끝나지 않았고,
  플랫폼 설정과 배포, 장애 대응까지 함께 살펴보아야 했습니다.
