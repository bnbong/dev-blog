...
이 함께 엮이는 도메인입니다. 그래서 리펙토링을 하면서 단일 앱 구현보다 도메인별 앱 분리와 공통 모듈 정리에 더 집중해 `attendance`, `event`, `timetable`, `utils`, `core`로 나눴습니다. 출결 로직을 한 파일에 몰아넣지 않고 역할 기준으로 쪼개려는 의도였습니다.
...
- 당시에는 백엔드 경험이 부족했던 터라 Django가 제가 선택할 수 있던 가장 최선의 stack이었습니다.
...
출결 도메인이 단순 요청/응답만으로 끝나지 않고 시간이 지나면서 상태를 갱신해야 하는 시스템이라 그렇게 잡았습니다.
...
- 문서와 스케줄러를 함께 관리해 본 경험이 이후 운영성 있는 백엔드 프로젝트에 도움이 됐습니다.
