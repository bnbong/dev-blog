...
`blog-post-workflow`는 README에 블로그 최신 글 목록을 자동으로 채워 넣어 주는 GitHub Action입니다.
이 프로젝트에는 두 번에 걸쳐 기여했습니다. 처음에는 NAVER Blog RSS feed 가이드를 공식 문서에 반영하는
작업이었고, 이후에는 `1.9.5` 릴리스에서 발생한 `item_exec` 회귀 버그를 원인까지 분석해 리포트했습니다.
...
이 기여는 코드 몇 줄을 바꾸는 작업이라기보다, 기존 자동화 워크플로우가 새로운 플랫폼을 어떻게 수용하는지
설명하는 성격이 강했습니다. 한국에서는 NAVER Blog가 실제 수요가 있는 소스였기 때문에,
공식 문서에 그 경로를 추가하는 것만으로도 의미가 있었습니다.
네이버 블로그는 Velog나 티스토리보다 개발자들이 잘 찾지 않는 소스이지만, 한국의 대표 블로그라 기여할 가치가 있다고 봤습니다.
...
단순 신고로 끝내지 않고 소스 차이를 직접 추적했습니다.

- `1.9.4`에서는 `eval(ITEM_EXEC)`로 `item_exec` 스니펫을 호출자 스코프 안에서 실행해서
  `customTags` 같은 로컬 변수에도 자연스럽게 접근할 수 있었습니다.
- `1.9.5`부터는 `new Function('post', ...)` 방식으로 바뀌면서 스코프가 분리됐고,
  결과적으로 `post`만 주입되어 `customTags`는 더 이상 참조할 수 없게 됐습니다.

기능 추가가 아니라 실행 컨텍스트 변경이 과거 사용자의 스니펫을 조용히 깨뜨린 회귀였습니다.
`customTags`가 여전히 `post`에 병합되기는 해서, `post.tag`로 우회할 수 있다는 것도 같이 확인했습니다.
...
Patch 릴리스(`1.9.4` → `1.9.5`) 안에서 호환성이 깨진 사례였기 때문에, 저처럼 `@v1`을 신뢰하고 고정해 둔
사용자 다수가 영향을 받을 수 있었습니다. 그래서 "제 워크플로우가 안 돌아간다"에서 멈추지 않고,
원인 커밋과 동작 차이까지 정리해 유지관리자가 바로 판단할 수 있는 형태로 제출했습니다.

## 배운 점

- 오픈소스 기여는 기능 추가뿐 아니라, 다른 사용자가 프로젝트를 더 쉽게 쓰게 만드는 문서 기여와
  회귀 리포트도 똑같이 중요합니다.
- 버그를 재현만 남기는 것과 원인 코드와 커밋까지 추적해 전달하는 것은 기여의 무게가 완전히 다릅니다.
